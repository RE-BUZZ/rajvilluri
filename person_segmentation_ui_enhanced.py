...
class PersonSegmentatorUI:
    ...
    def setup_ui(self):
        ...
        # Instructions
        if DRAG_DROP_AVAILABLE:
            instruction_text = "Drag & drop an image or click Browse to remove background"
        else:
            instruction_text = "Click Browse to select an image (install tkinterdnd2 for drag & drop)"
        
        instruction_label = ttk.Label(self.root, text=instruction_text, style='Info.TLabel')
        ...
        self.clear_btn = tk.Button(button_frame, text="🗑️ Clear", command=self.clear_images,
    ...
    def setup_drag_drop(self):
        ...
        if DRAG_DROP_AVAILABLE:
            # Enable drag and drop on the input canvas
            self.input_canvas.drop_target_register(DND_FILES)
            self.input_canvas.dnd_bind('<<Drop>>', self.on_drop)
            self.input_canvas.dnd_bind('<<DragEnter>>', self.on_drag_enter)
            self.input_canvas.dnd_bind('<<DragLeave>>', self.on_drag_leave)
        else:
    ...
    def on_drag_enter(self, event):
        ...
        self.input_canvas.configure(highlightcolor='#28a745', highlightthickness=3)
        
    def on_drag_leave(self, event):
        """Handle drag leave event"""
        self.input_canvas.configure(highlightcolor='#0078d4', highlightthickness=2)
    ...
    def load_image(self, file_path):
        ...
        try:
            ...
            # Update UI to show image has been loaded
            self.root.update()
            ...
            if canvas_width <= 1 or canvas_height <= 1:
                ...
                canvas_width, canvas_height = 580, 400
            ...
            # Add image info
            img_info = f"{os.path.basename(file_path)} ({image.width}x{image.height})"
            self.input_canvas.create_text(10, 10, text=img_info, fill='#ffffff', 
    ...
    def display_processed_image(self, processed_image):
        ...
        try:
            ...
            bg_image = self.create_checkered_background(canvas_width - 20, canvas_height - 20)
            
            # Resize processed image
            image_resized = self.resize_image_to_fit(processed_image, canvas_width - 20, canvas_height - 20)
            
            # Composite the image over the checkered background
            bg_image.paste(image_resized, ((canvas_width - 20 - image_resized.width) // 2,
                                         (canvas_height - 20 - image_resized.height) // 2), 
                          image_resized)
        ...
        try:
            ...
            if file_path:
                ...
                if file_path.lower().endswith('.png'):
                    ...
                    msg = f"✅ Transparent PNG saved successfully!"
                else:
                    ...
                    msg = f"✅ Image with white background saved successfully!"
...
def main():
    ...
    try:
        ...
        if not DRAG_DROP_AVAILABLE:
            print("pip install tkinterdnd2  # For drag and drop support")
